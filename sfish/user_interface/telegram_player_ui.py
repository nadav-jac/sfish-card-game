...
class TelegramPlayerUI:
    ...
    def get_move_from_user(self, player_cards: list[Card], board: Board):
        """
        ...
        self.bot.send_message(chat_id=self.user_id, text="Choose a card to play:", reply_markup=reply_markup)

    def get_guess_from_user(self, player_cards: list[Card], possible_guesses: list[int], strong_suit: Card):
        """
        ...
        self.bot.send_message(chat_id=self.user_id, text=f"Strong suit: {strong_suit}. How many sets will you win?", reply_markup=reply_markup)
